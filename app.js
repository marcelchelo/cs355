...
app.get('/TRNS_RULES/:id', (req, res) => {
  ...
  const queryString =
    'SELECT * FROM (SELECT Course_ID, Descr, Equiv_Crs FROM CRSE_CAT LIMIT 15000) A INNER JOIN (SELECT Descr, CRSE_ID FROM TRNS_RULES WHERE Descr = ?) B ON A.Course_ID = B.CRSE_ID'
