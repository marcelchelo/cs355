...
function addAnotherSchool() {
	if (numOfPastSchoolsSelection < 3) {
		...
		var newSchoolSelection = $(
			"<div class='dropdown dropdown-past-schools-div' id='past-schools-selection-container-'" +
				numOfPastSchoolsSelection +
				" style='display:none'> <button id='past-schools-" +
				numOfPastSchoolsSelection +
				"' class='btn btn-secondary dropdown-toggle dropdown_past_schools' type='button' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'> Find your past college or university </button> <div class='menu dropdown-menu' aria-labelledby='dropdown_past_schools'> <form class='px-4 py-2'> <input id='search-past-school-" +
				numOfPastSchoolsSelection +
	...
	if (!selectedSchools.includes(elem.value)) {
		var selectedSchool = $(
			"<div class='school' id='" +
				...
				"'> <h2> <span></span> " +
				elem.value +
				" </h2> <a class='close'> <img src='style/images/close-button.svg' alt='close button' class='close-button' align='middle'/> </a> </div>"
		);
...
function buildTransferDropdown(values) {
	...
	for (let school of values) {
		contents.push(
			'<input type="button" class="dropdown-item transfer-dropdown-item" type="button" value="' + school + '"/>'
		);
	}
...
document.getElementById('searchTransferSchool').addEventListener('input', function() {
